...
def filter_bank(nfft, nfilt, minFreq, maxFreq, sr):
    ...
    for i in range(nfilt):
        ...
        fbank[i, :] = maximum(zeros(loslope.shape),
                              minimum(loslope, highslope))
...
def to_mfcc(filename, num_coeffs=20, win_len=0.025, time_step=0.01, num_filters=26, freq_lims=(80, 7800), use_power=True, alpha=0.97, sr=None):
    ...
    filterbank = filter_bank(
        (pspec.shape[1]-1) * 2, num_filters, minHz, maxHz, sr)
...
def to_envelopes(path, num_bands, freq_lims, downsample=True):
    ...
    bandLo = [freq_lims[0]*exp(log(freq_lims[1]/freq_lims[0]) /
                               num_bands)**x for x in range(num_bands)]
    bandHi = [freq_lims[0]*exp(log(freq_lims[1]/freq_lims[0]) /
                               num_bands)**(x+1) for x in range(num_bands)]

    envelopes = []
    for i in range(num_bands):
        b, a = butter(
            2, (bandLo[i]/(sr/2), bandHi[i]/(sr/2)), btype='bandpass')

...
def xcorr_distance(rep_one, rep_two):
    ...
    matchSum = correlate(longer_rep[:, 0]/sqrt(sum(longer_rep[:, 0]**2)),
                         shorter_rep[:, 0]/sqrt(sum(shorter_rep[:, 0]**2)), mode='valid')
...
def distance_to_center(source):
    ...
    for i in range(sLen):
        for j in range(tLen):
            distMat[i, j] = euclidean(source[i, :], 0)
    return distMat
